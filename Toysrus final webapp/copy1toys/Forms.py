...
class AddToyForm(FlaskForm):
    name = StringField('Toy Name', [DataRequired(), Length(min=2, max=100)])
    ...
    image = StringField('Image Filename (e.g., toy.jpg)', [DataRequired()])

    def validate_price(self, field):
        try:
            float(field.data)
        except ValueError:
            raise ValidationError("Price must be a numerical value.")
